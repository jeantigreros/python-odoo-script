...
def create_success_response():
    """Create a proper SOAP success response for Odoo"""
...
def create_error_response(error_message):
    """Create a proper SOAP error response for Odoo"""
...
def print_text(text):
    ...
    try:
        ...
        FEED = b"\n\n\n"          # Feed 3 lines
        TXT_DOUBLE_SIZE = b"\x1D\x21\x11"  # Double height & width
        TXT_NORMAL_SIZE = b"\x1D\x21\x00"  # Normal size
        ...
        win32print.WritePrinter(hPrinter, INIT + TXT_DOUBLE_SIZE + data_bytes + FEED + FEED + TXT_NORMAL_SIZE + CUT)
...
def print_receipt():
    ...
    try:
        ...
        if img is None:
            error_response = create_error_response("Failed to extract image from request")
            return Response(error_response, status=200, mimetype='text/xml')
        ...
        if text is None:
            error_response = create_error_response("Failed to extract text from image")
            return Response(error_response, status=200, mimetype='text/xml')
        ...
        if print_text(text):
            ...
            return Response(success_response, status=200, mimetype='text/xml')
        else:
            error_response = create_error_response("Printing failed")
            return Response(error_response, status=200, mimetype='text/xml')
            
    except Exception as e:
        logger.error(f"Unexpected error: {e}")
        error_response = create_error_response(f"Server error: {str(e)}")
        return Response(error_response, status=200, mimetype='text/xml')
